...
class ExceptionContractSpec {
    ...
    fun `chunk head uses no such element on empty chunk`() {
        assertFailsWith<NoSuchElementException> { Chunk.empty<Int>().head }
    }
    ...
    fun `leftist heap empty access uses one empty element contract`() {
        ...
        assertFailsWith<NoSuchElementException> { empty.findMin() }
        assertFailsWith<NoSuchElementException> { empty.deleteMin() }
    }
    ...
    fun `float deque get set and iterator use collection exception types`() {
        ...
        assertFailsWith<IndexOutOfBoundsException> { deque[-1] }
        assertFailsWith<IndexOutOfBoundsException> { deque[1] = 2f }
    ...
    fun `float deque pop operations use collection exception types`() {
        ...
        assertFailsWith<NoSuchElementException> { empty.popFirst() }
        assertFailsWith<NoSuchElementException> { empty.popLast() }
        assertFailsWith<IndexOutOfBoundsException> { empty.popFirst(-1) }
        assertFailsWith<IndexOutOfBoundsException> { empty.popLast(1) }
    }
    ...
    fun `float buffer empty access and index-like arguments use collection exception types`() {
        ...
        assertFailsWith<NoSuchElementException> { buffer.removeFirst() }
        assertFailsWith<NoSuchElementException> { buffer.removeLast() }
        assertFailsWith<NoSuchElementException> { buffer.reduce { left, right -> left + right } }
        assertFailsWith<IndexOutOfBoundsException> {
            buffer.insertAt(0, floatArrayOf(1f), startIndex = -1)
        }
        assertFailsWith<IndexOutOfBoundsException> { buffer.insertAt(0, listOf(1f), endIndex = 2) }
    }
}
