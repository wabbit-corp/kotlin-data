...
value class NonEmptyList<out A> private constructor(val value: List<A>) : List<A> {
    ...
    override fun containsAll(elements: Collection<@UnsafeVariance A>): Boolean = value.containsAll(elements)
    override fun contains(element: @UnsafeVariance A): Boolean = value.contains(element)

    fun <B> map(transform: (A) -> B): NonEmptyList<B> {
        return NonEmptyList(value.map(transform))
    }

    companion object {
        fun <A> of(first: A, vararg rest: A): NonEmptyList<A> {
            return NonEmptyList(listOf(first) + rest.toList())
        }
        ...
        fun <A> fromListOrNull(value: List<A>): NonEmptyList<A>? {
            return if (value.isEmpty()) null else NonEmptyList(value)
        }
    }
}
